...
function builderIndexList( _sql ,_id ,_pId ,_opObj ){
    ...
    JbsManager.processIndexListSQL(me.sql,"",
        function(data){
            ...
            var indexdata = eval('('+data.split('|')[1]+')');
            var isTree = (indexdata.children !== undefined);
            for(var i = 0 ; i < columns.length -1 ; ++i ){
                ...
                if(isTree){
                    columnMArray[i].xtype = i==2 ? 'treecolumn' : '';
                    columnMArray[i].flex = i==2 ? 1 : undefined;
                    columnMArray[i].hidden = i<2 || (columns[i]=='checkbox') || columns[i] == 'seria_num' || columns[i] == 'document_num';
                }else{
                    columnMArray[i].hidden = (columns[i] == 'seria_num' || columns[i] == 'document_num');
                    columnMArray[i].filterable = true;
                    columnMArray[i].flex = i==0 ? 1 : undefined;
            ...
            if(isTree){
                me.indexlistStore = Ext.create('Ext.data.TreeStore', {
                    fields:fieldMArray,
                    root:indexdata,
                    listeners:{
...
/*            var fieldMArray = [];
            ...
            */
            ...
            if(!isTree){
                me.indexlistStore.add(indexdata.result);
            }else{
                me.indexlistStore.setRootNode(indexdata);
            }
