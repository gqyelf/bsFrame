...
function enterKeyEvents ( _obj ,_field ,_sqlProc ,_baseParam ,_value ,_extendParam1 ,_extendParam2 ,_row ,_column ){
    ...
    if(_sqlProc == '' || _sqlProc === undefined){
        ...
        var nextCell = _obj.getNextEditCellPosition(Ext.getCmp(objId) ,Ext.getCmp(objId).getSelectionModel().getCurrentPosition() ,_field );
        Ext.getCmp(objId).getSelectionModel().select(Ext.getCmp(objId).getStore().getAt(nextCell.row));
        Ext.getCmp(objId).getPlugin().startEditByPosition(nextCell);
...
function enterKeyEventsF ( _obj ,_field ,_sqlProc ,_baseParam ,_value ,_extendParam1 ,_extendParam2 ,_row ,_column ){
    ...
    if(_sqlProc == '' || _sqlProc === undefined){
        if(objColumns[nextPos.column].xtype=='textfield'||objColumns[nextPos.column].xtype=='numberfield'){
